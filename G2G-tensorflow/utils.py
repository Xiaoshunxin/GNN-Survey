...
def score_node_classification(features, z, p_labeled=0.1, n_repeat=10, norm=False):
    ...
    for seed in range(n_repeat):
        sss = StratifiedShuffleSplit(n_splits=1, test_size=0.073, train_size=0.015)
        ...
        p_macro = precision_score(z[split_test], predicted, average='macro')
        r_macro = recall_score(z[split_test], predicted, average='macro')
        accu = accuracy_score(z[split_test], predicted)

        trace.append((accu, p_macro, r_macro, f1_macro, f1_micro))
...
def load_dataset(file_name):
    ...
    with np.load(file_name) as loader:
        ...
        loader['adj_data']
            ...
            size: num_edges
        loader['adj_indices']
            value: [1638 2357 1636 ... 1865  745  200]
            size: num_edges
        loader['adj_indptr']
            value: [   0    3    6 ... 8413 8416 8416]
            size: num_edges + 1
        loader['adj_shape']
            value: [2995 2995]
       ...
       """
        ...
        """
        loader['attr_data']
            value: [0.08539068 0.10902652 0.1756043  ... 0.13603258 0.45228662 0.1460117 ]
            size: the sum of all num_features (151171)
        loader['attr_indices']
            value: [1421 1392 2282 ...  947 1789  951]
            size: the sum of all num_features (151171)
        loader['attr_indptr']
            value: [     0     85    133 ... 151103 151138 151171]
            size: num_nodes + 1
        loader['attr_shape']
            value: [2995 2879]
       ...
       """
        ...
        """
        loader.get('idx_to_attr')
            type: numpy.ndarray
            value: {0: u'000', 1: u'10',..., 2878: u'zero'}
            size: num_features
        idx_to_attr
            type: dict
            value: {0: u'000', 1: u'10',..., 2878: u'zero'}
       ...
       """
        ...
        """
        loader.get('idx_to_class')
            type: numpy.ndarray
            value: {0: u'Artificial_Intelligence/Machine_Learning/Case-Based',...,6: u'Artificial_Intelligence/Machine_Learning/Reinforcement_Learning'}
            size: num_classes
       """
...
def sample_all_hops(hops, nodes=None):
    ...
    """
        nodes:
            value: [   0    1    2 ... 2992 2993 2994]
            shape: (num_nodes,)
        hops[h].rows[nodes]
            type: numpy.ndarray
            value: [list([1636, 1638, 2357]) ... list([])]
            shape: (num_nodes,)
        "middle"
            type: numpy.ndarray
            value: [[1636 1955    1 ...  653  745   -1]]
            shape: (1, num_nodes)
        sample_last_hop(hops[-1], nodes)
            type: numpy.ndarray
            value: [2732 2607 1653 ...  508 2208 2273]
            shape: (num_nodes,)
        "result"
            type: numpy.ndarray
            shape: (3, num_nodes)
...
def sample_last_hop(A, nodes):
    ...
    sampled_nodes : array-like, shape [N]
        The sampled nodes.
    """
    N = A.shape[0]  # N = num_nodes

    """
        sampled
            type: numpy.ndarray
            value: [1893 2710 1234 ...  108 1193 1553]
            shape: (num_nodes, )
    """
    ...
    """
        A[nodes, sampled]
            ...
            value: (0, 2396)	1.0
            shape: (1, num_nodes)
        A[nodes, sampled].nonzero()
            type: tuple
            value: (array([0], dtype=int32), array([2396], dtype=int32)
        nnz
            type: numpy.ndarray
            value: [2396]
            shape: (1, )
    ...
    while len(nnz) != 0:
        new_sample = np.random.randint(0, N, len(nnz))  # new_sample = 1708
        sampled[nnz] = new_sample
        nnz = A[nnz, new_sample].nonzero()[1]

    # It's not connected for each node
    return sampled
